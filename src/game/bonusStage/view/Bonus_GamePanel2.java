...
public class Bonus_GamePanel2 extends JPanel implements Runnable {
	...
	public Bonus_GamePanel2(JFrame mf) {
		this.mf = mf;
		this.setLayout(null);
		panel = this;
	...
	private void shakeButton() {
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				for (int i = 0; i < 5; i++) {
					try {
						setButtonMargin(new Insets(margin.top, margin.left + 5, margin.bottom, margin.right - 4));
						Thread.sleep(delay);
						setButtonMargin(margin);
						Thread.sleep(delay);
						setButtonMargin(new Insets(margin.top, margin.left - 4, margin.bottom, margin.right + 5));
						Thread.sleep(delay);
						setButtonMargin(margin);
						Thread.sleep(delay);
