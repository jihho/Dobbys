...
/*	public void GamePlay(int harryChoice) {
		...
		do {
			...
			F_ChoiceResult result = new F_ChoiceResult();
			...
			case 0 :	//공격일때
				if(vmchoice == 0 || vmchoice == 1 || vmchoice == 2) {
					result.setChoice(0); 
					System.out.println("공격이 무효화 되었습니다.");break;
					...
					System.out.println("볼드모트의 체력 " + voldmort.getHp());break;
				}
			...
			case 2 :	//스매쉬일때
				if(vmchoice == 0 || vmchoice == 2) {
					...
					System.out.println("볼드모트의 체력이 20 감소합니다.");
				}else if(vmchoice ==3) {
					harry.setHp(harry.getHp() -30);
					harry.setHplife(harry.getHplife() -3);
					System.out.println("해리의 체력이 30 감소합니다.");
				}
			case 3 : 	//카운터일때
				if(vmchoice == 0) {
			...
			if(voldmort.getHp()<=0) {
				System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1볼드모트를 물리쳤습니다."); return;
	...
	public F_General harry(int hp, int hplife) {
		return harry;
	}
	
	public F_General voldmort(int hp, int hplife) {
		return voldmort;
	}*/
}
